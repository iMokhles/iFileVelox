...
#import <sys/types.h>
#import <sys/stat.h>
#import "iFileVeloxViewController.h"
#import "ZipArchive.h"

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunused-variable"

@interface iFileVeloxViewController ()
- (id)initWithPath:(NSString *)p;
@end

@implementation iFileVeloxViewController
...
- (id)initWithPath:(NSString *)p
{
	if ((self = [super initWithStyle:self.tableView.style])) {
		...
		contents = [[[IFVClient sharedInstance] IFV_contentsOfDirectory:path] mutableCopy];
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	[[IFVClient sharedInstance] IFV_fileExists:childPath];
	BOOL isDir = [[IFVClient sharedInstance] IFV_fileIsDirectory:childPath];

	cell.imageView.image = isDir ? [UIImage imageWithContentsOfFile:@"/Library/Application Support/IFVClient/IFVClient.bundle/folderIcon.png"] : [UIImage imageWithContentsOfFile:@"/Library/Application Support/IFVClient/IFVClient.bundle/fileIcon.png"];
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (isDir) {
		iFileVeloxViewController *child = [[iFileVeloxViewController alloc] initWithPath:childPath];
	    ...
	    if ([fileExt isEqualToString:@""]) {
	    	if (![fileArchiveZip openZipFile2:zipFileName withZipModel:APPEND_STATUS_ADDINZIP]) {
		    	...
		    	[[IFVClient sharedInstance] IFV_copyFile:childPath IFV_toFile:newFilePath];
	    	} else {
...
- (void)tableView:(UITableView *)tv commitEditingStyle:(UITableViewCellEditingStyle)es forRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (es == UITableViewCellEditingStyleDelete) {
		...
		if ([self.delegate shouldDeleteFileAtPath:childPath]) {
			...
			[[IFVClient sharedInstance] IFV_deleteFile:childPath];
